...
def process_excel_file(file_path):
    ...
    with pd.ExcelWriter('output.xlsx') as writer:
        ...
        # Store high matches in a text file
        ...
        # Store exact duplicates in an Excel sheet
        exact_duplicates_df = pd.DataFrame(columns=['Row', 'Number of Duplicates', 'Duplicate Rows', 'Matching Attributes'])
        processed_pairs = set()  # Keep track of processed pairs to avoid duplicates
        ...
        for index, row in exact_duplicates.iterrows():
            ...
            for dup_index in duplicate_indices:
                ...
                if pair not in processed_pairs:
                    ...
                    matching_attributes = ['Site Name']  # Always include site name for exact duplicates
                    for col in ['site_city', 'site_state', 'site_country']:
                        if len(df[df['site_name'] == row['site_name']][col].unique()) == 1:
                            matching_attributes.append(col.title())

                    new_row = pd.DataFrame({
                        'Row': [index + 1],
                        'Number of Duplicates': [len(duplicate_indices)],
                        'Duplicate Rows': [', '.join(str(dup + 1) for dup in duplicate_indices)],
                        'Matching Attributes': [', '.join(matching_attributes)]
                    })

                    exact_duplicates_df = pd.concat([exact_duplicates_df, new_row], ignore_index=True)

        # Store high matches in an Excel sheet
        high_matches_df = pd.DataFrame(columns=['Row', 'Number of Matches', 'Matching Rows', 'Matching Attributes'])
        processed_pairs = set()  # Keep track of processed pairs
                ...
                for match_index, match_row in matches.iterrows():
                    ...
                    if pair not in processed_pairs:
                        ...
                        new_row = pd.DataFrame({
                            ...
                            'Matching Rows': [', '.join(str(match + 1) for match in matches.index)],
                            'Matching Attributes': [', '.join(matching_attributes)]
                        })

                        high_matches_df = pd.concat([high_matches_df, new_row], ignore_index=True)

        # Write exact duplicates and high matches to Excel sheets
        exact_duplicates_df.to_excel(writer, sheet_name='Exact Duplicates', index=False)
        high_matches_df.to_excel(writer, sheet_name='High Matches', index=False)
